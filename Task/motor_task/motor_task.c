...
//	portENTER_CRITICAL();
	...
	//	can_motor_send(motor_stdid,0,0,1000,1000);
	can_motor_send(motor_stdid,0,0,0,chassis_.mt6816.pid_param_speed.out);
	} 
	else{
...
void StartDefaultTask(void const * argument)
{
  ...
  {	
		...
		if(chassis_.rc_contr.s[0] != 2 || chassis_.rc_contr.s[1] != 2)
			can_motor_send(motor_stdid,0,0,chassis_.motor_3508.pid_param_speed.out,chassis_.motor_2006.pid_param_speed.out);
